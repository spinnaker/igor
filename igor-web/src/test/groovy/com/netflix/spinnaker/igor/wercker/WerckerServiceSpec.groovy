...
class WerckerServiceSpec extends Specification {
	...
	void 'get pipelineId and builds'() {
		...
		List<Pipeline> pipelines = [pipeOf('anotherPipe', 'git'), pipeOf(pipe, 'git', pipelineId)]
	...
	void 'get builds with pipelineId'() {
		...
		def (pipelineId, pipeline) = ['myPipelineID', org + '/' + app + '/' + pipe]
		def now = System.currentTimeMillis();
		List<Run> runs = [
	...
	void 'test triggerBuildWithParameters'() {
		...
		setup:
		client.getPipelinesForApplication(_, org, app) >> [pipeOf('foo', 'git'), pipeOf(pipe, 'git', pipelineId)]
